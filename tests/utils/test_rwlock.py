...
class Test_RWLock():
    def test_readers_nonexclusive_access(self):
        ...
        assert [] == threads[0].buffer_read
        assert [1] == threads[2].buffer_read
        assert [1] == threads[3].buffer_read
        assert threads[1].exit_time <= threads[2].entry_time
        assert threads[2].entry_time <= threads[3].entry_time
        assert threads[3].exit_time < threads[2].exit_time
    ...
    def test_writers_exclusive_access(self):
        ...
        assert [1, 2] == threads[2].buffer_read
        assert threads[0].exit_time <= threads[1].entry_time
        assert threads[1].exit_time <= threads[2].exit_time
    ...
    def test_writer_priority(self):
        ...
        assert [1] == threads[1].buffer_read
        assert [1, 2] == threads[3].buffer_read
        assert [1, 2] == threads[4].buffer_read
        assert threads[0].exit_time < threads[1].entry_time
        assert threads[1].exit_time <= threads[2].entry_time
        assert threads[2].exit_time <= threads[3].entry_time
        assert threads[2].exit_time <= threads[4].entry_time
    ...
    def test_many_writers_priority(self):
        ...
        assert [1] == threads[1].buffer_read
        assert [1, 2, 3] == threads[3].buffer_read
        assert [1, 2, 3] == threads[4].buffer_read
        assert threads[0].exit_time < threads[1].entry_time
        assert threads[1].exit_time <= threads[2].entry_time
        assert threads[1].exit_time <= threads[5].entry_time
        assert threads[2].exit_time <= threads[3].entry_time
        assert threads[2].exit_time <= threads[4].entry_time
        assert threads[5].exit_time <= threads[3].entry_time
        assert threads[5].exit_time <= threads[4].entry_time
